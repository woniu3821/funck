<template>
    <div>
        <Index></Index>
        <el-upload class="avatar-uploader" name="img" :action="serverUrl" :data={uid:useruid} :show-file-list="false" :on-success="uploadSuccess" :on-error="uploadError" :before-upload="beforeUpload">
        </el-upload>
    ...
    uploadSuccess(res, file) {
      ...
      if (file.status === "success" && file.response !== null) {
        ...
        quill.insertEmbed(
          length,
          "image",
          `${file.response.destination.replace(/static/, "")}/${
